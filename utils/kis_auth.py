...
def get_token_path(svr):
    return os.path.join(config_root, f"KIS_{svr}_{datetime.today().strftime('%Y%m%d')}")

# 접근토큰 관리하는 파일 존재여부 체크, 없으면 생성
# (This part is moved to inside auth/read_token to handle dynamic path)
...
try:
    ...
    # 파일이 없을 경우 예외 처리 (개발 중에는 없을 수 있음)
    _cfg = {}
...
def save_token(my_token, my_expired, svr):
    token_tmp = get_token_path(svr)
...
def auth_ws(svr="prod", product=_cfg.get("my_prod", "01")):
...
def reAuth_ws(svr="prod", product=_cfg.get("my_prod", "01")):
    n2 = datetime.now()
...
class KISWebSocket:
    ...
    async def __subscriber(self, ws: websockets.ClientConnection):
        async for raw in ws:
            ...
            if raw[0] in ["0", "1"]:
                ...
                try:
                    # Use usecols to avoid errors if data has more columns than expected
                    # and names to assign column names to the first N columns
                    df = pd.read_csv(
                        StringIO(d), 
                        header=None, 
                        sep="^", 
                        names=dm["columns"], 
                        usecols=range(len(dm["columns"])),
                        dtype=object
                    )
                    show_result = True
                except Exception as e:
                    print(f"Error parsing WebSocket data: {e}")
                    # print(f"Raw data: {d}") # Optional: print raw data for debugging
                    continue
        ...
        while self.retry_count < self.max_retries:
            try:
                async with websockets.connect(url) as ws:
                    self.ws_conn = ws # Keep reference for dynamic subscription
                    
                    # request subscribe (Initial)
    ...
    def subscribe(
        ...
        if hasattr(self, 'ws_conn') and self.ws_conn is not None:
            # We need to schedule this coroutine in the running loop
            # Since subscribe is called from sync context, we need to use the loop
            try:
                loop = asyncio.get_running_loop()
                # Create a task to send the subscription
                loop.create_task(self.send_multiple(self.ws_conn, request, "1", data, kwargs))
                logging.info(f"Dynamic subscription requested for {data}")
            except RuntimeError:
                # No running loop (maybe called before start), just adding to map is enough
                pass
