...
class MovingAverageTrendStrategy(BaseStrategy):
    def on_bar(self, symbol, bar):
        ...
        if position is not None:
            ...
            if abs(pnl_pct) > 1.0:
                 self.logger.info(f"[{symbol}] 보유 중 | 현재가: {int(current_price):,}원 | 수익률: {pnl_pct:.2f}% (목표: {self.config['take_profit1_pct']}%)")
            ...
            if position.partial_taken and current_price < avg_price:
                 self.logger.info(f"[{symbol}] 본전 탈출 (Breakeven) 실행 | 현재가: {int(current_price):,}원 < 평단가")
            ...
            if pnl_pct <= -self.config["stop_loss_pct"]:
                self.logger.info(f"[{symbol}] 손절매 (Stop Loss) 실행 | 수익률: {pnl_pct:.2f}% (조건: -{self.config['stop_loss_pct']}%)")
                self.broker.sell_market(symbol, position.qty, tag=self.config["id"])
                return
            ...
            if max_price > 0:
                ...
                if drawdown_from_high <= -self.config["trail_stop_pct"]:
                    self.logger.info(f"[{symbol}] 트레일링 스탑 실행 | 고점 대비 하락: {drawdown_from_high:.2f}% (고점: {int(max_price):,}원)")
                    self.broker.sell_market(symbol, position.qty, tag=self.config["id"])
        ...
        if daily is None or len(daily) < 20:
             self.logger.warning(f"[데이터 부족] {symbol} | 일봉 데이터 부족 ({len(daily) if daily is not None else 0}/20)")
             return
        ...
        if not trend_up:
             self.logger.info(f"[감시 제외] {symbol} | 사유: 하락 추세 (현재 {int(daily.close.iloc[-1]):,} < MA20 {int(ma20_daily_now):,})")
             return

        # Trend is UP -> Check Intraday Data
        if bars is None or len(bars) < 20:
             self.logger.info(f"[감시 중] {symbol} | 대기: 데이터 수집 중 (분봉 {len(bars) if bars is not None else 0}/20)")
             return
        ...
        if avg_vol20 > 0:
             ...
             self.logger.info(f"[감시 중] {symbol} | 현재가: {int(bars.close.iloc[-1]):,} | 이평돌파: {int(ma5_now):,} ≥ {int(ma20_now):,}{ma_stat} | 거래량비: {vol_ratio:.1f} ≥ {vol_k:.1f}{vol_stat}")
        ...
        if golden_cross and not vol_ok:
             self.logger.info(f"[{symbol}] 골든크로스 발생했으나 거래량 부족 (현재: {volume_now}, 필요: {int(avg_vol20 * self.config.get('vol_k', 1.5))})")

        if golden_cross and vol_ok:
            qty = self.calc_position_size(symbol)
            if self.risk.can_open_new_position(symbol, qty):
                self.logger.info(f"[{symbol}] 매수 진입 (골든크로스 + 거래량 충족) | 수량: {qty}주 | 현재가: {int(bars.close.iloc[-1]):,}원")
