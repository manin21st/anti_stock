...
class Investor:
    ...
    def watch(self):
        ...
        for symbol in self.target_universe:
            ...
            if self.check_watch_condition(symbol):
                ...
                logger.info(f"[{symbol}] 감시 조건 미달 -> 패스")
    ...
    def entry(self, symbol):
        ...
        if self.check_entry_condition(symbol):
            logger.info(f"[{symbol}] 진입 조건 만족 -> [매수 주문] 실행")
            self.buy(symbol)
        else:
            logger.info(f"[{symbol}] 진입 조건 미충족")
    ...
    def exit(self, symbol):
        ...
        if self.check_exit_condition(symbol):
            logger.info(f"[{symbol}] 청산 조건 만족 -> [매도 주문] 실행")
            self.sell(symbol)
        else:
            logger.info(f"[{symbol}] 청산 조건 미충족")
    ...
    def buy(self, symbol):
        """매수 실행"""
        logger.info(f"  >>> {symbol} 매수 완료!")

    def sell(self, symbol):
        """매도 실행"""
        logger.info(f"  >>> {symbol} 매도 완료!")
